                    ...
                    <div class="box-body">
                        ...
                        <div style="margin: auto" class="row col-md-10 mx-auto order-send-status my-3">
                            @foreach(config('store.sendStatus') as $statusId => $status)
                                ...
                                ?>
                                <form id="s-{{ $orderShippingId }}-s-{{ $statusId }}" onSuccess="changeStatus" action="{{ url('api/orders/setStatus') }}" method="post" class="ajaxForm item {{ $st == null ? '' : 'active' }} col-md-3 tac">
                                    @csrf
